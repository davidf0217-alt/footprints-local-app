...
function CityExplorer({ city, record, onClose, initialView }) {
  const [tab, setTab] = useState("sights"); const [view, setView] = useState(initialView || "guide");
  const guide = CITY_GUIDES[city];
  useEffect(() => { const closeOnEscape = (event) => { if (event.key === "Escape") onClose(); }; addEventListener("keydown", closeOnEscape); return () => removeEventListener("keydown", closeOnEscape); }, [onClose]);
  if (!city) return null;
  ...
  return <div className="city-modal" role="dialog" aria-modal="true" aria-label={`${city}城市探索`}>
    <button type="button" className="city-modal-backdrop" aria-label="关闭城市探索" onClick={onClose} />
    <section className="city-modal-card">
      <button type="button" className="city-modal-close" aria-label="关闭" onClick={onClose}>×</button>{view === "plan" ? <TripPlanCard city={city} guide={guide} onBack={() => setView("guide")} /> : <>
      <p className="eyebrow">CITY SCOOP</p>
      <div className="city-modal-title"><div><h2>{city}</h2><p>{guide?.tone || "城市灵感卡"} · {record ? "已收进雪糕柜" : "未到先逛逛"}</p></div><span>{record ? "🍨" : "🧭"}</span></div>
...
export default function MapDashboard({ records, onCollect, requestedCity, onCityOpened }) {
  ...
  useEffect(() => { if (requestedCity) { setSelectedCity(requestedCity); setOpenPlan(true); onCityOpened?.(); } }, [requestedCity, onCityOpened]);
  ...
  return <><section className="stats">{stats.map((item) => <div className="stat" key={item.label}><strong>{item.value}</strong><span>{item.label}</span></div>)}</section><section className="map-wrap" aria-busy={state === "loading"}>{state === "loading" && <div className="status">正在打开雪糕地图…</div>}{state === "error" && <div className="status error">地图暂时加载失败，请检查网络后重试。</div>}<div ref={chartRef} className="chart" hidden={state !== "ready"} /></section><section className="controls"><button className="button primary" onClick={() => { if (playIndex < 0 || playIndex >= timeline.length - 1) setPlayIndex(0); setPlaying(true); }} disabled={!timeline.length}>{playing ? "正在回味" : playIndex >= 0 ? "继续回味" : "回味旅程"}</button><button className="button secondary" onClick={() => setPlaying(false)} disabled={!playing}>先放一放</button><button className="button secondary" onClick={() => { setPlaying(false); setPlayIndex(-1); }}>看看全部</button></section>{current && <div className="progress" aria-live="polite"><span>{current.date}　{current.city}　{current.days} 天</span><i style={{ "--progress": `${((playIndex + 1) / timeline.length) * 100}%` }} /></div>}<section className="achievement" aria-label="城市解锁成就"><div className="achievement-main"><span className="achievement-icon">{currentAchievement?.icon || "🍦"}</span><div><p>已解锁 {unlockedCities.length} 座城市</p><h2>{currentAchievement?.title || "第一口，先出发"}</h2><span>{currentAchievement?.note || "记录第一座城市即可解锁称号。"}</span></div></div>{nextAchievement ? <div className="achievement-next"><span>距离「{nextAchievement.title}」还差 {nextAchievement.min - unlockedCities.length} 座</span><i><b style={{ width: `${Math.min(100, (unlockedCities.length / nextAchievement.min) * 100)}%` }} /></i></div> : <p className="achievement-complete">全部称号已点亮，继续为你的雪糕柜添新口味吧。</p>}<div className="achievement-badges">{ACHIEVEMENTS.map((item) => <span className={unlockedCities.length >= item.min ? "is-unlocked" : ""} key={item.title} title={`${item.min} 座城市解锁`}>{item.icon}<b>{item.title}</b></span>)}</div></section><section className="city-discoveries"><div className="section-heading"><div><p>LOCAL FLAVOURS</p><h2>城市图鉴</h2></div><span>抵达后才会解锁成就；图鉴可先浏览和做计划</span></div>{cityGuides.length ? <div className="city-guide-grid">{cityGuides.map(({ city, guide, unlocked }) => <button className="city-guide" key={city} onClick={() => { setOpenPlan(false); setSelectedCity(city); }}><div className="city-guide-title"><div><span>{unlocked ? "已解锁" : "待探索"} · {guide.tone}</span><h3>{city}</h3></div><i>{unlocked ? "🍨" : "🧭"}</i></div><div className="guide-pairs"><p><b>🍜 吃什么</b>{guide.foods.join(" · ")}</p><p><b>📍 去哪里</b>{guide.sights.join(" · ")}</p></div><span className="guide-open">打开探索卡 →</span></button>)}</div> : <p className="empty">城市图鉴正在准备中。</p>}</section><section className="footprints"><h2>我的雪糕柜</h2><div>{records.length ? records.map((item) => <button className={`chip${active.includes(item) ? " is-active" : ""}`} key={item.id} onClick={() => { setOpenPlan(false); setSelectedCity(item.city); }}>🍨 {item.city} <b>{item.date}</b></button>) : <p className="empty">雪糕柜还是空的，去加第一球旅程吧。</p>}</div></section><CityExplorer city={selectedCity} record={selectedRecord} initialView={openPlan ? "plan" : "guide"} onClose={() => { setOpenPlan(false); setSelectedCity(null); }} /></>;
}
