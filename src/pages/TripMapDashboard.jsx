...
export default function TripMapDashboard() {
  const hashRoute = () => location.hash.slice(1) === "scan" ? "scan" : location.hash === "#collect" ? "collect" : "dashboard";
  const [route, setRoute] = useState(hashRoute); const [records, setRecords] = useState(readRecords); const [onboarded, setOnboarded] = useState(() => localStorage.getItem("xuegao.onboarded.v1") === "1"); const fileRef = useRef(null);
  ...
  const handleImport = async (event) => { const file = event.target.files?.[0]; if (!file) return; try { setRecords(importRecords(await file.text())); alert("旅行碎片已安全合并进雪糕柜。"); } catch { alert("无法导入：请选择由雪糕足迹导出的有效 JSON 备份文件。"); } event.target.value = ""; };
